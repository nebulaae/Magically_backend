...
export const publishFromGallery = async (galleryItemId: string, userId: string) => {
  const t = await db.transaction();
  try {
    const galleryItem = await Gallery.findByPk(galleryItemId, { transaction: t, lock: true });
