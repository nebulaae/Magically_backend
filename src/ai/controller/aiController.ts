...
export const generateImage = async (req: Request, res: Response) => {
    const { prompt, modelId, publish, aspect_ratio, width, height, seed, safety_tolerance } = req.body;
    const userId = req.user.id;
    ...
    // ⚠️ НЕ СПИСЫВАЕМ ТОКЕНЫ ЗДЕСЬ! Они спишутся после успешной генерации в поллере
    try {
        ...
        // Создаём job с информацией о провайдере
        ...
        apiResponse.success(res, { jobId: job.id, provider: result.provider }, "Generation started.");
    } catch (error: any) {
