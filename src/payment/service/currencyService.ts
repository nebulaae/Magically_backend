...
export const getCurrencyByIP = async (ip: string | null): Promise<{
  currency: string;
  country: string | null;
  ip: string | null;
}> => {
  ...
  try {
    ...
    if (response.data.status === "fail") {
      logger.warn(`Failed to get geolocation for IP ${ip}: ${response.data.message}`);
      return {
        currency: "RUB",
        country: null,
        ip,
    ...
    logger.info(`Currency determined for IP ${ip}: ${currency} (country: ${countryCode})`);
