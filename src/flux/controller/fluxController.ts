...
export const createModel = async (req: Request, res: Response) => {
  ...
  const model = await fluxService.createFluxModel(
    userId,
    name,
    description,
    instruction,
...
export const updateModel = async (req: Request, res: Response) => {
  ...
  const model = await fluxService.updateFluxModel(
    req.user.id,
    modelId as string,
...
export const getModel = async (req: Request, res: Response) => {
  const { modelId } = req.params;
  const model = await fluxService.getFluxModelById(
    req.user.id,
    modelId as string
  );
...
export const generateImage = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const fluxResponse = await fluxService.generateFluxImage(
      userId,
      modelId,
      prompt,
      aspect_ratio
