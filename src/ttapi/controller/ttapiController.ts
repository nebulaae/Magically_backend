...
export const createModel = async (req: Request, res: Response) => {
  ...
  const model = await ttapiService.createTtModel(
    userId,
    name,
    description,
    instruction,
...
export const updateModel = async (req: Request, res: Response) => {
  ...
  const model = await ttapiService.updateTtModel(
    userId,
    modelId as string,
...
export const generate = async (req: Request, res: Response) => {
  const { prompt, modelId, publish, width, height, seed, safety_tolerance } =
    req.body;
  const userId = req.user.id;
